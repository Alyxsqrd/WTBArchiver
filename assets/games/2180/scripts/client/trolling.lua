...
function OnTouchBegin(wildcard)
    if isHost then
        -- if we're the host and our character touches the checkpoint, update our checkpoint in the trollingManager
...
function OnNetMessage(data)
    -- if we recieved a request to update a player's checkpoint, then we should update the player's checkpoint in the trollingManager
    if isHost then
...
function SetCheckpoint(player)
    local trollingManager = GetObjectByName("TrollingManager")
    if IsValid(trollingManager) then
        -- only update the checkpoint if they aren't already on this checkpoint
        if not trollingManager.netTable["player"..player.netId] or trollingManager.netTable["player"..player.netId] ~= object.id then
            trollingManager.netTable["player"..player.netId] = object.id
            -- let the player know that they reached a checkpoint
            player.SendSystemChat("Absolutely bonkers!")
        end
    end
end