...
roundBeginTime = 5
roundEndTime = 10
disaster_table = {"Acid Flood", "Man Eating Cube", "Asteroid Storm", "Four Corners", "Hole In The Wall", "Alien Abduction", "Spin Zone"}
...
function Begin()
    ...
    while true do
        ...
        if disOverride == 'none' then
            if currentDisaster == 'Acid Flood' then
                SendSystemChatToAll('Rules: Avoid the acid as it rises')
                WaterFlood()
            ...
            if disOverride == 'Acid Flood' then
                SendSystemChatToAll('OVERRIDE Acid Flood')
                WaterFlood()
    ...
    for i = 1,#players do
        if IsValid(object.localTable[players[i].netId.."alive"]) then
            if object.localTable[players[i].netId.."alive"] == true then
                ...
                --give win rewards
                -- only function is 1
                -- stat 1 = wins, stat 2 = points
                --("function", playernetId, stat, amount)
                local rewardTable = {1,players[i].netId,1,1}
                GetObjectByName("leaderboardManager").NetMessagePlayer(GetHostPlayer(), rewardTable)
                local rewardTable = {1,players[i].netId,2,3}
                GetObjectByName("leaderboardManager").NetMessagePlayer(GetHostPlayer(), rewardTable)
                counter = counter+1
            end
        end
...
function WaterFlood()
    ...
    speed = 0.4 * 70 / duration --units per second
    for i = 1,roundBeginTime do
        wait(1)
        roundTimeRemaining = roundTimeRemaining- 1
        updateRoundTimer(roundTimeRemaining)
