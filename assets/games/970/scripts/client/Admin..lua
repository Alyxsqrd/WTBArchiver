--[[     ___   __         _        _____                              __
	  ...
	  Script by Alyx (binary#1337) | v2021.04.09 | Support server: TenrBdJ ]]
...
AddHandler("Fly", function(sender, data)
	...
	if switch == true and not GetLoop("Fly", 0) then
		Shared.OriginalMS = LP.speed

		LP.SetMovementMode("NoClip")

		AddLoop("Fly", 0, function()
			if InputHeld("left shift") then
				LP.speed = Shared.OriginalMS * 6
			end

			if InputReleased("left shift") then
				LP.speed = Shared.OriginalMS
			end
		end)
	elseif switch == false and GetLoop("Fly", 0) then
		RemoveLoop("Fly", 0)

		LP.SetMovementMode("Normal")
		LP.speed = Shared.OriginalMS

		Shared.OriginalMS = nil
	end
end)
...
function GrantAdmin(player)
	if not player or GetPlayerLevel(player) >= Ranks.Administrators.Level then
		return
	end

	table.insert(Ranks.Administrators.Members, player.WTBID)
end