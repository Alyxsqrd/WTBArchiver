...
function OnTouchBegin(touched)
    ...
    if (object.renderer.visible == true) then
        if purchased == false then 
            ...
            local dropperName = "Tycoon"..tycoonid.."Dropper2"
            ...
            local upgradeTextName = "Tycoon" .. tycoonid .. "Upgrade4Text"
            local upgradeText = GetObjectByName(upgradeTextName)
            if tycoon.netTable.Money >= price then
                --upgrade
                purchased = true
                dropper.netTable.dropNumber = dropper.netTable.dropNumber + 2
                ...
                -- Next Upgrade Unlock
                local nextUpgradeName = "Tycoon"..tycoonid.."Upgrade5"
                local nextUpgrade = GetObjectByName(nextUpgradeName)
                nextUpgrade.renderer.visible = true
                nextUpgrade.collider.enabled = true
                local nextUpgradeTextName = "Tycoon"..tycoonid.."Upgrade5Text"
                local nextUpgradeText = GetObjectByName(nextUpgradeTextName)
                nextUpgradeText.worldText.enabled = true

                local nextUpgradeName = "Tycoon"..tycoonid.."Unlock2"
                local nextUpgrade = GetObjectByName(nextUpgradeName)
                nextUpgrade.renderer.visible = true
                nextUpgrade.collider.enabled = true
                local nextUpgradeTextName = "Tycoon"..tycoonid.."Unlock2Text"
                local nextUpgradeText = GetObjectByName(nextUpgradeTextName)
                nextUpgradeText.worldText.enabled = true
                
                print('upgrade 4 purchased')
                print('dropper 2 amount is now '..dropper.netTable.dropNumber)
            end
        end
    end
end