    ...
    if (object.renderer.visible == true) then
        if purchased == false then 
            ...
            if tycoon.netTable.Money >= price then
                --upgrade
                purchased = true
                dropper.netTable.dropValue = dropper.netTable.dropValue + 5
                ...
                local nextUpgradeName = "Tycoon"..tycoonid.."Upgrade3"
                local nextUpgrade = GetObjectByName(nextUpgradeName)
                nextUpgrade.renderer.visible = true
                nextUpgrade.collider.enabled = true
                local nextUpgradeTextName = "Tycoon"..tycoonid.."Upgrade3Text"
                local nextUpgradeText = GetObjectByName(nextUpgradeTextName)
                nextUpgradeText.worldText.enabled = true

                local nextUpgradeName = "Tycoon"..tycoonid.."Unlock1"
                local nextUpgrade = GetObjectByName(nextUpgradeName)
                nextUpgrade.renderer.visible = true
                nextUpgrade.collider.enabled = true
                local nextUpgradeTextName = "Tycoon"..tycoonid.."Unlock1Text"
                local nextUpgradeText = GetObjectByName(nextUpgradeTextName)
                nextUpgradeText.worldText.enabled = true

                local nextUpgradeName = "Tycoon"..tycoonid.."Decor1"
                local nextUpgrade = GetObjectByName(nextUpgradeName)
                nextUpgrade.renderer.visible = true
                nextUpgrade.collider.enabled = true
                local nextUpgradeTextName = "Tycoon"..tycoonid.."Decor1Text"
                local nextUpgradeText = GetObjectByName(nextUpgradeTextName)
                nextUpgradeText.worldText.enabled = true
