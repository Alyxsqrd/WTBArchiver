...
function OnTouchBegin(touched)
    ...
    if (object.renderer.visible == true) then
        if purchased == false then 
            ...
            local upgradePartsName = "Tycoon"..tycoonid.."Floor2Part"
            ...
            local upgradeTextName = "Tycoon" .. tycoonid .. "Decor4Text"
            ...
            if tycoon.netTable.Money >= price then
                ...
                -- Next Upgrade Unlock
                local nextUpgradeName = "Tycoon"..tycoonid.."Decor5"
                local nextUpgrade = GetObjectByName(nextUpgradeName)
                nextUpgrade.renderer.visible = true
                nextUpgrade.collider.enabled = true
                local nextUpgradeTextName = "Tycoon"..tycoonid.."Decor5Text"
                local nextUpgradeText = GetObjectByName(nextUpgradeTextName)
                nextUpgradeText.worldText.enabled = true

                local nextUpgradeName = "Tycoon"..tycoonid.."Conveyor2"
                local nextUpgrade = GetObjectByName(nextUpgradeName)
                nextUpgrade.renderer.visible = true
                nextUpgrade.collider.enabled = true
                local nextUpgradeTextName = "Tycoon"..tycoonid.."Conveyor2Text"
                local nextUpgradeText = GetObjectByName(nextUpgradeTextName)
                nextUpgradeText.worldText.enabled = true
                
                print('floor 2 purchased')
            end
        end
    end
end